...
class SlidingUpPanel extends StatefulWidget {
  ...
  /// Optional predicate that determines whether a drag starting at the given
  /// global position should control the panel.
  ///
  /// This is useful when parts of the panel contain interactive content
  /// (e.g. charts) that should handle vertical drags themselves without
  /// moving the panel.
  ///
  /// Return `true` to allow the panel to react to the drag, or `false` to
  /// ignore this drag in the panel.
  final bool Function(Offset globalPosition)? dragStartPredicate;
...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  // Whether the current gesture (if any) should be allowed to move the panel.
  bool _shouldHandleDrag = true;
                      ...
                      Positioned(
                        ...
                        child: SizedBox(
                          ...
                          child: widget.collapsed == null
                              ...
                              : FadeTransition(
                                  opacity:
                                      Tween<double>(begin: 1, end: 0).animate(_ac),
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    void handleDragStart(DragStartDetails details) {
      if (widget.dragStartPredicate != null) {
        _shouldHandleDrag = widget.dragStartPredicate!(details.globalPosition);
      } else {
        _shouldHandleDrag = true;
      }
    }

    void handleDragUpdate(DragUpdateDetails dets) {
      if (_shouldHandleDrag) {
        _onGestureSlide(dets.delta.dy);
      }
    }

    void handleDragEnd(DragEndDetails dets) {
      if (_shouldHandleDrag) {
